    ...
    private function getEmployeeWidgetData()
{
    ...
    // Mengambil data transaksi per bulan berdasarkan tanggal transaksi untuk pegawai
    $transaksi_perbulan = Transaksi::select(
        DB::raw("DATE_FORMAT(tanggal, '%Y-%m') as month_year"),
        DB::raw('count(*) as count')
    )
    ...
    // Ensure the key is always present, even if not applicable for employee users
